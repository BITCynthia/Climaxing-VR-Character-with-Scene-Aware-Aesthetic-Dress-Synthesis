...
import labelScene.event as event

import globalVariance as GV
import fileProcess.eventWrite as eventWrite

# 路径
inputDir = "input/100"
# inputDir = "input/single"
outputDir = "output/event"

# 创建一个界面窗口
win = tkinter.Tk()
win.title("Labeling event... ")
...
def nextImg():
    eventWrite.save(outputDir, GV.fileList[GV.ID])
    GV.ID += 1
    render()
...
def main():
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~事件选择~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    label1 = tkinter.Label(win, text="1.事件")
    label1.place(x=nameX, y=220)

    button1_1 = tkinter.Button(win, text="日常", command=event.dailyLife)
    button1_1.place(x=nameXX[0], y=210)

    button1_2 = tkinter.Button(win, text="运动", command=event.sport)
    button1_2.place(x=nameXX[1], y=210)

    button1_3 = tkinter.Button(win, text="商务", command=event.business)
    button1_3.place(x=nameXX[2], y=210)

    button1_4 = tkinter.Button(win, text="旅行", command=event.trip)
    button1_4.place(x=nameXX[3], y=210)

    button1_5 = tkinter.Button(win, text="宴会", command=event.party)
    button1_5.place(x=nameXX[4], y=210)

    button1_6 = tkinter.Button(win, text="艺术照", command=event.photo)
    button1_6.place(x=nameXX[5], y=210)

    button1_7 = tkinter.Button(win, text="其他", command=event.other)
    button1_7.place(x=nameXX[6], y=210)
