...
chromosomeLength = 23  # 染色体长度
scope = [(1, 5), (1, 16), (1, 5), (1, 5), (1, 11), (1, 7), (1, 4), (1, 3), (1, 5), (1, 6), (1, 4),
         (1, 10), (0, 3), (1, 4), (1, 3), (1, 6), (1, 4),
         (1, 7), (0, 3), (1, 6), (1, 2), (1, 7), (1, 4)]
...
def mutateForbid(chromosomeIndex, mutateIndex):
    # upper
    if chromosomeIndex[0] == 1:
        if mutateIndex >= 1 and mutateIndex <= 10:
            return False
    # trousers
    if chromosomeIndex[0] == 2:
        if mutateIndex >= 11 and mutateIndex <= 16:
            return False
    # skirt
    if chromosomeIndex[0] == 3:
        if mutateIndex >= 17 and mutateIndex <= 22:
            return False
    # dress
    if chromosomeIndex[0] == 4:
        if (mutateIndex >= 1 and mutateIndex <= 10) or (mutateIndex >= 17 and mutateIndex <= 22):
            return False
    # Jumpsuits
    if chromosomeIndex[0] == 5:
        if mutateIndex >= 1 and mutateIndex <= 16:
            return False

    return True


def constrain(chromosome):
    if chromosome[0] == 1:  # upper
        for i in range(11, 23):
            chromosome[i] = 0
    else:
        if chromosome[0] == 2:  # trousers
            for i in range(1, 11):
                chromosome[i] = 0
            for i in range(17, 23):
                chromosome[i] = 0
        else:
            if chromosome[0] == 3:  # skirt
                for i in range(1, 17):
                    chromosome[i] = 0
            else:
                if chromosome[0] == 4:  # lyq
                    for i in range(11, 17):
                        chromosome[i] = 0
                else:
                    if chromosome[0] == 5:  # ltk
                        for i in range(17, 23):
                            chromosome[i] = 0
...
def mutation(newChromosomeMatrix):
    ...
    while mutateForbid(newChromosomeMatrix[chromosomeIndex], mutateIndex):
        mutateIndex = random.randint(0, chromosomeLength - 1)

    # 变异后的值
    mutateValue = random.randint(scope[mutateIndex][0], scope[mutateIndex][1])
...
def initialGeneration(outfit):
    chromosomeMatrix = []
    for i in range(chromosomeNum):
        chromosomeMatrix_i = []
        chromosomeMatrix_i.append(outfit)
        for j in range(chromosomeLength - 1):
            chromosomeMatrix_i.append(random.randint(scope[j][0], scope[j][1]))
        chromosomeMatrix_i = constrain(chromosomeMatrix_i.copy())
        chromosomeMatrix.append(chromosomeMatrix_i)
    return chromosomeMatrix


def geneAlgorithm(figure, scene, outfit):
    print(" 1.Generating the primary population...")
    chromosomeMatrix = initialGeneration(outfit)
    ...
    for i in range(iteratorNum):
        ...
        #print("    ", "chromosomes ", chromosomeMatrix[0])
...
def optimize():
    ...
    outfit = of.outfit(sceneIDList[sceneNo], figureIDList[figureNo])

    print("II.Optimizating...")
    time_start = time.time()
    clothSequence = geneAlgorithm(figure, scene, outfit)
    time_end = time.time()
    ...
    sg.storeLabel(value, os.path.join(outputDir, sceneIDList[sceneNo] + '_' + figureIDList[figureNo] +
                                      '_detail1.json'))
